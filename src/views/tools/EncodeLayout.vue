    ...
    <a-row :gutter="24">
        ...
        <a-col :span="12">
            <a-card :title="`${currentEncoder.name} 编解码`">
                ...
                <a-button type="primary" block size="large" @click="handleConvert" :loading="converting">
                    <template #icon>
        ...
        <a-col :span="12">
            <a-card title="处理结果">
                <div class="result-container">
                    <a-textarea v-model:value="outputText"
                        :placeholder="mode === 'encode' ? currentEncoder.resultLabel : currentEncoder.decodeResultLabel"
                        :auto-size="{ minRows: 8, maxRows: 12 }" readonly class="result-area" />
                    <div class="copy-button-wrapper">
                        <a-tooltip title="复制结果">
                            <a-button type="link" @click="copyResult" :disabled="!outputText" class="copy-btn">
                                <copy-outlined />
                            </a-button>
                        </a-tooltip>
                    </div>
...
const handleConvert = async () => {
    try {
        ...
        const processor = mode.value === 'encode'
            ? currentEncoder.value.encode
            : currentEncoder.value.decode;
