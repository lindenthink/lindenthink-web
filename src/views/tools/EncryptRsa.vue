  ...
  <a-row :gutter="24">
    ...
    <a-col :span="12">
      ...
      <a-card title="加解密操作" style="margin-top: 24px">
        <a-tabs v-model:active-key="operationType">
          <a-tab-pane key="encrypt" tab="加密">
            ...
            />
            <a-button type="primary" block style="margin-top: 16px" @click="handleEncrypt"> 执行加密 </a-button>
          </a-tab-pane>
          <a-tab-pane key="decrypt" tab="解密">
            ...
            />
            <a-button type="primary" block style="margin-top: 16px" @click="handleDecrypt"> 执行解密 </a-button>
          </a-tab-pane>
        </a-tabs>
      </a-card>
    </a-col>
...
const exportKey = async (key, type) => {
  ...
  return (
    `-----BEGIN ${type === 'public' ? 'PUBLIC' : 'PRIVATE'} KEY-----\n` +
...
const handleEncrypt = async () => {
  ...
  try {
    ...
    const encrypted = await window.crypto.subtle.encrypt({ name: 'RSA-OAEP' }, publicKeyObj, encoded)
...
const handleDecrypt = async () => {
  ...
  try {
    ...
    const decrypted = await window.crypto.subtle.decrypt({ name: 'RSA-OAEP' }, privateKeyObj, rawData)
