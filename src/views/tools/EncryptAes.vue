    ...
    <a-col :span="12">
      <a-card title="AES加解密配置">
        <a-form layout="vertical">
          ...
          <a-space>
            <a-button type="primary" :disabled="mode === 'decrypt'" @click="handleEncrypt">
              <lock-outlined /> 加密
            </a-button>
            <a-button type="primary" :disabled="mode === 'encrypt'" @click="handleDecrypt">
              <unlock-outlined /> 解密
            </a-button>
            <a-button @click="swapMode"> <swap-outlined /> 切换模式 </a-button>
          </a-space>
        </a-form>
      </a-card>
    </a-col>
      ...
      <a-card title="处理结果">
        <div class="result-container">
          <a-textarea
            ...
            :auto-size="{ minRows: 6, maxRows: 10 }"
            :placeholder="mode === 'encrypt' ? 'Base64加密结果' : '解密后文本'"
            readonly
            class="result-area"
          />
          <div class="copy-button-wrapper">
            <a-tooltip title="复制结果">
              <a-button type="link" :disabled="!outputText" @click="copyResult">
                <copy-outlined />
              </a-button>
            </a-tooltip>
          </div>
...
const handleDecrypt = () => {
  try {
    ...
    const decrypted = CryptoJS.AES.decrypt(inputText.value, key, {
      iv: ivBytes,
      mode: CryptoJS.mode[config.value.mode],
      padding: CryptoJS.pad.Pkcs7,
    })
