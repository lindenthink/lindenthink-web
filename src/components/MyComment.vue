<template>
  <div class="comment">
    ...
    <a-comment :author="user.name">
      <template #avatar>
        <a-avatar :src="user.avatar" :alt="user.name">游客</a-avatar>
      </template>
      <template #content>
...
const data = [
  {
    ...
    datetime: dayjs('2022-07-16 22:54'),
  },
  {
    actions: ['回复'],
    author: 'Han Solo',
...
watch(isShowQuote, (newValue, oldValue) => {
  placeholder.value = newValue ? `@${quoteItem.author}` : '请输入回复内容'
})
